...
export class ContactsService {
  ...
  async findOne(id: string, userId: string): Promise<Contact> {
    ...
    if (participantError || !participant) {
      throw new NotFoundException(`Conversation with ID ${id} not found or user is not a participant`)
    }

    // Get the other participant in this conversation
    const { data: otherParticipant, error: otherParticipantError } = await this.supabaseService.client
      .from("participants")
      .select("user_id")
      .eq("conversation_id", id)
      .neq("user_id", userId)
